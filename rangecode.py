...
for _, row in df.iterrows():
    if row['range1'] < range_min:
        compressed_data.append({'range1': f'< {range_min}', 'range2': f'< {range_min}', 'CCY': row['CCY'], 'notional': row['notional']})
    elif row['range2'] > range_max:
        compressed_data.append({'range1': f'> {range_max}', 'range2': f'> {range_max}', 'CCY': row['CCY'], 'notional': row['notional']})
    else:
